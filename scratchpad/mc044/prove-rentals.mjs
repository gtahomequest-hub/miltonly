// MC-044 proof 3 (a copy of ML-012's scratchpad/ml012/measure.mjs, run against production after the batch):
...
await page.setUserAgent('miltonly-verify mc044');
...
for (const v of VIEWS) {
  ...
  const d = await page.evaluate(() => {
    ...
    const all = [...new Set([...document.querySelectorAll('a[href^="/"]')].map((a) => a.getAttribute('href').split('#')[0]).filter(Boolean))];
    return { cards: cards.length, withLink, withoutLink, withheld, plainStreet, links, all };
  });
  result.views[v] = { status, ...d };
}
const distinct = [...new Set(Object.values(result.views).flatMap((v) => [...v.links, ...v.all]))];
for (const href of distinct) {
  try {
    const r = await fetch(`${BASE}${href}`, { redirect: 'manual', headers: { 'user-agent': 'miltonly-verify mc044' } });
...
for (const d of Object.values(result.views)) {
  ...
  d.allCount = d.all.length; d.all404 = d.all.filter((h) => linkStatus.get(h)?.status === 404); d.allNon2xx = d.all.filter((h) => { const s = linkStatus.get(h)?.status; return !(s >= 200 && s < 300); }).map((h) => `${h} ${linkStatus.get(h)?.status}`); delete d.all;
}
result.distinctLinks = distinct.length;
await browser.close();
fs.mkdirSync('scratchpad/mc044', { recursive: true });
fs.writeFileSync(`scratchpad/mc044/rentals-measure-${LABEL}.json`, JSON.stringify(result, null, 1));
...
console.log(`distinct links fetched (card street links plus every internal link on the four views): ${distinct.length}`);
for (const [v, d] of Object.entries(result.views)) console.log(`${v}: every internal link: ${d.allCount}, 404 ${d.all404.length}${d.all404.length ? ' ' + d.all404.join(' ') : ''}; non-2xx ${d.allNon2xx.length}${d.allNon2xx.length ? ' ' + d.allNon2xx.slice(0, 12).join(', ') : ''}`);
