...
2. **TYPICAL PRICE WITH SAMPLE CONTEXT** — state `input.neighbourhoodComparable.typicalSoldPrice` with rounding per the rounding tables. Cite `input.neighbourhoodComparable.sampleSize` ONLY if it adds meaning (k≥10 = "full" can stand without explicit citation; k between 5 and 9 = "thin" should acknowledge the smaller sample). Do NOT cite the raw integer count as prose ("142 sales"). Use editorial framing.
   ...
   **PHRASING GUARD — read before writing the price (this is where thin / sub-k streets fail-close).** `typicalSoldPrice` is a neighbourhood AGGREGATE. Surface it as a SINGLE tier-rounded point with aggregate framing, and in NO other form:
   - **Single point only — never a band or range.** Write one figure: "comparable {propertyType} homes across {neighbourhood} typically trade around $825,000." A low–high band ("$780,000 to $810,000," "the high-$700s to low-$800s," "in the $700s," "settles in the $Y range," "$X to $Y") is FORBIDDEN and trips the sub-k range gate (`subk_range_reassembly`). One number, not two — do not pair `typicalSoldPrice` with any other figure to imply a spread.
   - **Aggregate framing only — never singular per-trade phrasing.** Attach the price to the POOL ("comparable homes typically trade around $X," "the typical sold price across {neighbourhood} sits near $X"), never to one transaction ("a comparable home sold for $X," "one nearby townhome closed at $X," "a property in the area traded at $X"). A singular determiner (a / an / one) + trade verb + nearby price trips `per_trade_fabrication` even when $X is exactly the input figure.
   - **Tier form — follow the rounding table, and mind the $1M boundary.** Round the figure to its tier multiple ($10K under $500K, $25K from $500K–$1M, $50K from $1M–$2M). **At $1,000,000 and above, write the M-suffix or tier-band form — "around $1.1M," "the low-$1Ms," "the mid-$1.3Ms" — and NEVER the bare-comma $1,234,000 form.** The validator reads the leading "$1,064" of a "$1,064,000" comma figure as an un-rounded *rent* and fires `precise_price`; the M-suffix form ("$1.3M") cannot be mis-parsed and is the only safe way to state a $1M+ comparable. Below $1M the "around $825,000" comma form is fine.
   This guard applies regardless of `kAnonLevel` and is the only acceptable way to surface the neighbourhood price in this section.
