...
| `video` | every street page carrying a clip prints the takedown address; every clip carries a coverage sentence stating what it is a pass along and when it was filmed on its own clock, sits at a dated R2 key with its poster beside it, does not preload, and serves (clip and poster HEADed); a night clip stating a time states one after 8 pm; the VideoObjects mirror the players one for one; `sitemap-video.xml` serves and lists exactly the pages with a clip and exactly their clips |
