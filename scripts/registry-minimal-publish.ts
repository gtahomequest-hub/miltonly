...
//
// StreetContent.status is the ONLY thing this writes, and that is now the whole story: surfacing
// derives publication from it (src/lib/streetSurface.ts). The entity flag this script used to set
// alongside it, hasPublishedPage, was dropped 2026-08-17 — it was a copy that drifted.
...
async function main() {
  ...
  if (MODE === "draft") {
    ...
    console.log(`-> set ${r.count} minimal pages to DRAFT (removed from sitemap + static params, and unsurfaced — surfacing reads this status).`);
  } else {
    const r = await p.streetContent.updateMany({ where: { template: "minimal" }, data: { status: "published", publishedAt: new Date() } });
    console.log(`-> set ${r.count} minimal pages to PUBLISHED (${slugs.length} slugs; surfacing follows automatically).`);
  }
