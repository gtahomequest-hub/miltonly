...
const MIGRATION = "20260719013000_seo_digest_metrics";
...
async function main() {
  ...
  try {
    ...
    const check = await c.query(
      `SELECT table_name FROM information_schema.tables WHERE table_schema='public' AND table_name IN ('SeoOpportunity','SenseRun') ORDER BY table_name`
    );
