#!/usr/bin/env node
// MA-005. Internal links IN to each audit hub, counted from the live corpus.
//
//   BASE=https://miltonly.com node scripts/audit/hub-inbound.mjs
//
...
const STREETS = JSON.parse(fs.readFileSync(path.join(HERE, 'hubs.json'), 'utf8')).map((s) => s.slug);
...
async function worker() {
  while (queue.length) {
    const u = queue.shift();
    try {
      const r = await fetch(u, { headers: { 'user-agent': 'miltonly-audit/MA-005' } });
      if (!r.ok) { console.log(`${r.status} ${u}`); continue; }
      const raw = await r.text();
      // BODY LINKS ONLY: the footer names every hub and the header carries the menu, so both are stripped before matching.
      const html = raw.replace(/<footer[\s\S]*?<\/footer>/gi, '').replace(/<header[\s\S]*?<\/header>/gi, '').replace(/<nav[\s\S]*?<\/nav>/gi, '');
      const from = new URL(u).pathname;
      for (const m of html.matchAll(hrefRe)) {
        const href = m[1];
        for (const s of STREETS) {
          if (href === `/neighbourhoods/${s}` || href.startsWith(`/neighbourhoods/${s}#`) || href === `${BASE}/neighbourhoods/${s}`) {
            if (from !== `/neighbourhoods/${s}`) wanted.get(s).push({ from, text: m[2].replace(/<[^>]+>/g, '').replace(/\s+/g, ' ').trim().slice(0, 80) });
          }
        }
      }
...
for (const [s, links] of wanted) {
  ...
  out[s] = { inbound: byFrom.size, onSitemap: onSitemap.has(`/neighbourhoods/${s}`), sources: [...byFrom].map(([from, texts]) => ({ from, texts: [...new Set(texts)] })) };
  console.log(`${s}: ${byFrom.size} linking pages, on sitemap ${onSitemap.has(`/neighbourhoods/${s}`)}: ${[...byFrom.keys()].slice(0, 8).join(', ')}`);
}
