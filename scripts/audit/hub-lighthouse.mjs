#!/usr/bin/env node
// MA-005. Lighthouse over the audit hubs, mobile (slow 4G, Moto G class) and desktop presets.
//
//   BASE=https://miltonly.com LH_BIN=<path to lighthouse cli.js> node scripts/audit/hub-lighthouse.mjs [--only=slug]
//
...
const OUT = (process.argv.find((a) => a.startsWith('--out=')) || '').slice(6) || path.join(HERE, '..', '..', 'scratchpad', 'audit', 'MA-005', 'lh');
const only = (process.argv.find((a) => a.startsWith('--only=')) || '').slice(7).split(',').filter(Boolean);
fs.mkdirSync(OUT, { recursive: true });
const STREETS = JSON.parse(fs.readFileSync(path.join(HERE, 'hubs.json'), 'utf8')).filter((s) => !only.length || only.includes(s.slug));

const rows = [];
for (const s of STREETS) {
  for (const preset of ['mobile', 'desktop']) {
    const file = path.join(OUT, `${s.slug}.${preset}.json`);
    const args = [LH, `${BASE}/neighbourhoods/${s.slug}`, '--output=json', `--output-path=${file}`, '--quiet',
