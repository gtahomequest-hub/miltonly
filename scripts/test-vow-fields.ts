...
ok(/getSession\(\)/.test(gridPage) && /canSeeVowRecords\(/.test(gridPage), "/listings reads the session server-side through the one VOW judge");
...
ok(/force-dynamic/.test(route) && /getSession\(\)/.test(route) && /canSeeVowRecords\(/.test(route), "the VOW route is force-dynamic and gated by the one VOW judge");
ok(/canSeeVowRecords\(/.test(code("src/app/api/auth/saved-listings/route.ts")), "saved-listings gates status by the one VOW judge");
