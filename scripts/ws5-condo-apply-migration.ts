// WS5 condo persistence — apply the migration via pg (Prisma Rust engine hits
// Neon cold-start P1001 from this machine; pg connects reliably). Clone of
// scripts/ws3-apply-migration.ts with two adaptations:
//   1. Host guard at the live DB1 host (ep-patient-paper) + explicit --prod flag.
//   2. CRLF→LF normalization BEFORE hashing AND applying, so the recorded
//      checksum matches the committed LF bytes regardless of Windows checkout.
// Runs migration.sql in a transaction, records it in _prisma_migrations with
// the Prisma-matching sha256, idempotent skip if already recorded.
// Usage: npx tsx scripts/ws5-condo-apply-migration.ts --prod
...
const MIGRATION = "20260603210000_ws5_condo_persistence";
...
async function main() {
  ...
  // LF-normalize before hashing AND applying — the committed file is LF; a
  // CRLF Windows checkout must not change the recorded checksum.
  ...
  try {
    ...
    const check = await c.query(
      `SELECT table_name FROM information_schema.tables WHERE table_schema='public' AND table_name IN ('CondoGeneration','CondoContent') ORDER BY table_name`
    );
