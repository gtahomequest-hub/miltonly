...
// Moved 2026-05-09 to src/lib/slugMalformedDetection.ts so importers
// (e.g. scripts/generate-canonical-map.ts) can use isMalformedSlug
// without triggering this script's top-level main() side-effect.
...
async function processOne(
  ...
  // Defense-in-depth canonicalization, mirroring the guard at
  // generateStreetContent (src/lib/generateStreet.ts:265). Without this,
  // claimRow()'s raw SQL INSERT bypasses the guard and writes abbreviated
  // streetSlug values directly. That hole produced 7 duplicate SG rows on
  // 2026-05-09 (3-side-rd, 106-rottenburg-crt, agnew-cres, abbott-st,
  // aird-crt, alexander-cres, asleton-blvd).
  const { deriveIdentity } = await import("@/lib/streetUtils");
  const canonicalSlug = deriveIdentity(slug)?.canonicalSlug ?? slug;
  if (canonicalSlug !== slug) {
    log("slug_canonicalized_at_backfill_entry", { from: slug, to: canonicalSlug });
    slug = canonicalSlug;
  }
