...
export function queryMocks(query: ListingsQuery): ListingsV2Data {
  ...
  // Mock coordinates, spread across Milton so the design harness has something to lay out.
  // The real loader takes these from Listing.townLat/townLng — the resolved municipal rooftop —
  // and drops any listing without one; a card no longer carries a coordinate at all.
  const mockCoord = (i: number) => ({ latitude: 43.49 + (i % 7) * 0.006, longitude: -79.92 + (i % 5) * 0.012 });
  const mapPins: MapPin[] = rows.map((l, i) => ({
