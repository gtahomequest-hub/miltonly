...
function buildSystemPromptV2(streetName: string, cityName: string = "Milton", cityProvince: string = "Ontario"): string {
  return `You are Aamir Yaqoob, a RE/MAX Hall of Fame realtor in ${cityName}, ${cityProvince} with 15 years of experience selling homes street-by-street. You're writing a comprehensive guide for ${streetName} for Miltonly.com. Buyers, investors, sellers, and renters all read this page.
...
// descriptive call. Sections 5-7 (gettingAround/schools/differentPriorities)
// plus the FAQ are generated by an evaluative call. (bestFitFor retired
// 2026-07-19 — fair housing.)
// Calls run in parallel via Promise.all; results combine into the canonical
// 7(+1)-section + FAQ output and run through the existing post-processors and
 ...
 *      Evaluative (sections 5-7 + FAQ): gettingAround, schools,
...
export async function generatePhase41StreetContent(
  ...
  // homes, amenities, gettingAround, schools, differentPriorities)
...
OTHER SECTIONS (about, homes, amenities, gettingAround, schools, differentPriorities) write fully as designed. The numerical-avoidance constraint applies ONLY to the market section.
...
// Fallback pass: when AI_PROVIDER_FALLBACK is set (e.g. "opus" or "sonnet")
  ...
  // Combine into canonical 7(+1)-section + FAQ output. Sections must be in
  // canonical order: about, homes, amenities, market, (neighbourhoodComparable,)
  // gettingAround, schools, differentPriorities. ahaRes.sections gives the first
  // three; marketRes the fourth (+ optional comparable); evalRes the last three;
  // evalRes.faq the FAQ block.
