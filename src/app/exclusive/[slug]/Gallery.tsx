...
export default function Gallery({ photos, title }: Props) {
  ...
  if (photos.length === 0) {
    return (
      <div className="w-full aspect-[16/9] bg-[#0b3d2e] flex items-center justify-center">
        <p className="text-white/75 text-[14px]">No photos yet</p>
      </div>
    );
  }
  ...
  return (
    <>
      {/* Main photo */}
      <div className="relative bg-[#0b3d2e]">
      ...
      {/* Thumbnail strip */}
      {photos.length > 1 && (
        <div className="bg-white border-b border-[#dfe0dc]">
            ...
            {photos.map((p, i) => (
              <button
                ...
                className={`shrink-0 h-[80px] w-[110px] rounded-lg overflow-hidden border-2 transition-colors ${
                  i === activeIdx ? "border-[#00ff80]" : "border-transparent hover:border-[#dfe0dc]"
                }`}
