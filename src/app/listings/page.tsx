...
export const metadata = genMeta({
  ...
  description: `Browse ${config.CITY_NAME} ${config.CITY_PROVINCE} homes for sale. View listing photos, property details, and neighbourhood data. Live TREB MLSÂ® data updated daily.`,
  ...
  const faqs = [
    {
      ...
      answer: `There are currently ${totalCount} homes ${statusLabel} in ${config.CITY_NAME}, ${config.CITY_PROVINCE}. Listings update daily from TREB MLSÂ® data and include detached homes, semis, townhouses, and condos across every ${config.CITY_NAME} neighbourhood.`,
    },
    {
      ...
      <div className="min-h-screen bg-white">
        ...
        <div className="bg-white border-b border-[#e2e8f0] px-5 sm:px-11 py-5">
          <div className="max-w-7xl mx-auto">
            ...
            <p className="text-[14px] text-[#64748b] mb-4">{totalCount} homes Â· Live TREB MLSÂ® data</p>
            ...
            <form className="flex flex-wrap items-center gap-2">
              ...
              <span className="text-[11px] text-[#94a3b8]">â€“</span>
            ...
            <div className="max-w-7xl mx-auto">
              ...
              <div className="flex flex-wrap gap-2">
                {topStreets.map((s) => (
                  ...
                  >
                    {s.streetName}
                    <span className="text-[#94a3b8] ml-2">Â· {s._count} active</span>
                  </Link>
                ))}
              </div>
            </div>
        ...
        <footer className="bg-[#07111f] px-5 sm:px-11 py-12 border-t border-[#1e3a5f]">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="border-t border-[#1e3a5f] pt-6 text-[12px] text-[#64748b] text-center">
              {config.realtor.name} Â· {config.brokerage.name.replace(", Brokerage", "")} Â· {config.CITY_NAME} {config.CITY_PROVINCE} Â· <a href={`tel:${config.realtor.phoneE164}`} className="text-[#f59e0b] hover:underline">{config.realtor.phone}</a>
            </div>
            <p className="text-[10px] text-[#475569] text-center mt-4 max-w-3xl mx-auto leading-relaxed">
              Data provided by TREB via {config.SITE_NAME}. MLSÂ® listings updated daily. Information is deemed reliable but not guaranteed.
            </p>
          </div>
        </footer>
      </div>
