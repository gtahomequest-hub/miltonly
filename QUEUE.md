...
*Out-of-queue work 2026-09-11, MCT-001: two source-grounded guides, `/guides/parking-in-milton` (the Town's seven parking pages, fetched and stored under `src/data/sources/milton-parking/`, every rule sentence cited with URL and read date) and `/guides/milton-go-train-to-toronto` (computed from a stored GO GTFS feed, version and validity dates on the page). **BUILT AND PROVEN ON PREVIEW on `feat/content-2`, awaiting Core's merge.** Not a queue item, prompted directly. Record in `scratchpad/reports/MCT-001-parking-and-go-guides.md`, state in `HANDOFF-content.md`.*
