...
*Out-of-queue work 2026-09-19: MC-030, the return to production, blocked: production and every preview answered 402 for the whole task, so `main` (`c83fffb`, MC-029 merged as `5994cc7`) stays unpushed. Prepared on `prep/mc030` instead and gated locally (build exit 0; `next start` battery `PASS · 21 checks · 626 pages · 647s`): MP-002b `feat/portal @ 51a9bf9` merged as `9c98903`, ML-005 `feat/leads @ 064c5b6` merged as `fc05ae0`, and `d85a701` (MC-029's gates judge through `canSeeVowRecords`; the battery signs in with `VOW_BATTERY_PASSWORD`). Open: the desk account's password was changed by another session after the battery set one, so `vow-fields` fails by name until the desk puts the password in `.env.local` or names a battery account. Record in `scratchpad/reports/MC-030-production-return.md`.*

*Out-of-queue work 2026-09-18: MC-029, the TRREB VOW compliance hotfix, on `fix/vow-compliance @ de199c2` (code head `6c23bdc`), **merged on local `main` as `5994cc7` on 2026-09-19 and NOT pushed: Vercel was paused (402); gated locally (build exit 0; `next start` battery, four hub-page drifts purged and rerun clean), preview pending, push when production answers 200.** The seven VOW-only per-listing columns (`daysOnMarket`, `listedAt`, `priorPrice`, `priceChangedAt`, `lastPriceChangeAt`, `soldPrice`, `soldDate`) leave every anonymous surface through `src/lib/listings/vow.ts` (the public predicate and `stripVowFields`); an acknowledged session reads them from `/api/listings/[mls]/vow` on the listing page and on the grid's cards; sold, expired and leased listing pages answer the not-available shell, noindex; `/listings?status=sold` redirects to `/sold`; the brokerage renders inside the price on every card, the detail page and the alert email through one component. Battery `vow-fields` (red on main, 8 of 15; green on the branch), prebuild `test-vow-fields.ts` (67). Preview `czlzy3wc7` full battery `PASS · 21 checks · 609 pages · 769s`. Record in `scratchpad/reports/MC-029-vow-compliance.md`.*
