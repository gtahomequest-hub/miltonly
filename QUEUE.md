...
*Out-of-queue work 2026-09-20: MH-009, Vercel Web Analytics, in the `D:\miltonly-home` worktree on `feat/web-analytics` (cut from `origin/main @ d068f84`). `@vercel/analytics 2.0.1` mounted once in the root layout through a client component that owns `beforeSend`: auth paths lose their whole query, token-bearing keys become `redacted`, an unparseable URL is dropped, nothing renders outside `NODE_ENV=production`. Proven on the wire on the preview: the magic-link token, the preview secret and a pasted `token=` never reach Vercel. Adds one dependency (lockfile changes). Previewed at `7cfa4a2` (`--only=homepage,nav` `PASS · 2 checks · 646 pages · 172s`), NOT merged; Core merges by SHA. Record in `scratchpad/reports/MH-009-web-analytics.md`.*
