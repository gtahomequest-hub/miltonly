...
*Out-of-queue work 2026-09-23, MA-010, the two noisy nightly rules NARROWED in the `D:miltonly-audit` worktree on `feat/audit`. The premise was 1,674 findings (em-dash 1,368 + superlative 306 of 2,442), not ~1,800. The em-dash rule reads prose only: nav, breadcrumbs, control labels, value placeholders and link separators are out, and a DOM cross-check agrees on all 1,343 production pages; 1,368 findings become 574, 479 of them one sentence in `AgentContactSection.tsx:24`. The superlative rule fails closed: six exemptions, void when the sentence names the registrant. Best Road is read from the registry, and share cards, JSON-LD, alt, aria-label and own-brokerage remarks are now read; 333 findings become 7. The standing test `scripts/audit/nightly/test-voice-rules.mjs` passes 1,032 of 1,032 and runs first in the nightly. Full re-run: 1,526 open (S2 30). Top ten buried findings for Core, Home and Leads. Link-unpublished untouched, no rule disabled, not merged; code `794ef5a`. Record in `scratchpad/reports/MA-010-voice-rules.md`, state in `HANDOFF-audit.md`.*
