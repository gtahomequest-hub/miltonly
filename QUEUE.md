...
*Out-of-queue work 2026-09-21, MCT-002: the 37 published street descriptions MC-036 found naming a leased price tied to a month, regenerated through `scripts/regen-058-local.ts` on MC-036's prompt and input builder (main merged first, `4ee5f1f`). **36 of 37 published, $0.3878; `millside-drive-milton` fail-closed on `invented_cross_street` ("Dorset Park") and the Opus fallback could not run, the Anthropic account has no credit.** MC-036's test re-run: 1 of 691 remains, that page. **Second run on Aamir's yes: the 10 the broader check still named all published, $0.0980, no escalation; millside-drive scrubbed by hand outside the session; both tests 0 of 710. MCT-002 in all $0.4858, 47 pages.** Condos untouched (MC-037). Record in `scratchpad/reports/MCT-002-regenerate-lease-month-streets.md`, state in `HANDOFF-content.md`.*

*Out-of-queue work 2026-09-11, MCT-001: two source-grounded guides, `/guides/parking-in-milton` (the Town's seven parking pages, fetched and stored under `src/data/sources/milton-parking/`, every rule sentence cited with URL and read date) and `/guides/milton-go-train-to-toronto` (computed from a stored GO GTFS feed, version and validity dates on the page). **MERGED as `94ddc49`, up-links in `f01a96a`, on production. Re-issued 2026-09-13 and re-verified on preview `miltonly-5y3m3eeaf` at `98718b5`, battery `PASS · 16 checks · 509 pages`, not rebuilt.** Not a queue item, prompted directly. Record in `scratchpad/reports/MCT-001-parking-and-go-guides.md`, state in `HANDOFF-content.md`.*
