...
*Out-of-queue work 2026-09-10, the LEAD LAYER. **Phase 0 done**, merged as `c4a162b`: the two street forms that captured nothing, and `ALERT_EMAIL_TO`. **Phase 1 done**, merged as `3461e13`, a two-parent merge of `feat/leads`: one lead model, one guarded submission path, source-specific confirmations, watches for the alert surfaces, the alert cron, leads-per-page, and the environment tag. Not queue items — both were prompted directly. Records in `scratchpad/reports/062-leads-gate-a.md` and `063-unnotified-leads.md`, state in `HANDOFF-leads.md`. **Phase 2 done**, merged as `543ef99` (merges `26381f9`; ML-001 fast-forwarded `feat/leads` to `00e0eb1` and set `BRIEF_UNSUBSCRIBE_SECRET`): all twenty submission points on one client helper, `/api/leads` + `/api/off-market-leads` + `/api/exclusive-inquiry` deleted, a prebuild gate that walks `src/`, and the Monday-to-Friday daily-brief sender with a signed one-click unsubscribe. Record in `scratchpad/reports/067-leads-phase2.md`. **ML-003 built, awaiting Core's merge** (`feat/leads`): the weekly leads digest, Monday 07:00 Toronto to the desk, with a delivery log on every send. Record in `scratchpad/reports/ML-003-weekly-leads-digest.md`. **ML-004 built 2026-09-17, merged by Core as `7196623` (MC-028), deploy pending the Vercel pause, gated locally at `def48e6`** (`feat/leads`, proven tree `7eeeb79`): CASL on every recurring email (shared footer with the brokerage mailing address, signed one-click unsubscribe for any watch kind, List-Unsubscribe headers, proven on preview for the brief, a deal alert and the digest), `consentText` on all 26 lead submissions with a prebuild gate that fails a surface without it, and the street alert card says "listed for sale" because the cron matches listings only. Record in `scratchpad/reports/ML-004-casl-consent.md`. **ML-005 built 2026-09-19, gated locally, preview pending, awaiting Core's merge** (`feat/leads`, head `064c5b6`): the door's guard set on the lead ingress with the Gmail alias collapse, daily windows and a user-agent guard; 100 bot submissions write 0 rows and send 0 emails at prebuild; the 64 sale-detail bot rows purged. Record in `scratchpad/reports/ML-005-bot-gate.md`. **ML-012 built 2026-09-24, gated locally (exit 0, 841 of 841; local battery `PASS · 24 checks · 719 pages · 593s`), one preview `miltonly-ahq9hojts` at `4bc9343` (`PASS · 24 checks · 719 pages · 617s`), awaiting Core's merge** (`feat/leads`, proven tree `4bc9343`, above it only the docs commit and a docs-only merge of `origin/main` at `c636e20`; carries ML-005): the rental cards' muted ramp given a ground (the specs 1.0:1 to 4.89:1, five aaff821 literals on the deep ground 1.0:1 to 4.0:1 now 5.4:1 to 8.0:1) and the card's street link resolved on the server to the published page or nothing (114 links to 404 pages across four views to 0, 13 cards with no link). A live production defect: merges alone under DEC-BATCH-MERGE. Record in `scratchpad/ml012/ML-012-rentals-cards.md`.*
...
*Out-of-queue work 2026-09-22, MA-009, the three Lighthouse regressions of the 09-22 nightly, DIAGNOSED in the `D:miltonly-audit` worktree on `feat/audit`. Read-only against production at `c1565b7`: three Lighthouse runs each on `/neighbourhoods/timberlea` (94, 94, 95 against the nightly's 42), `/streets/woodward-avenue-milton` (90, 94, 92 against 78) and `/` (85, 89, 91 against 68). Runner noise, concentrated in the first three runs of each night, which by the harness's fixed order are those three pages; the analytics script never fires and the hub's prose shrank, so neither named change moved a resource. Nothing fixed, by instruction. Record in `scratchpad/reports/MA-009-lighthouse-regressions.md`, state in `HANDOFF-audit.md`.*

*Out-of-queue work 2026-09-21, MA-008, the morning report skeleton, BUILT in the `D:miltonly-audit` worktree on `feat/audit`. `scripts/audit/morning/` (Vercel dollars and traffic, Neon compute and egress with every project labelled by its reader, GSC through the service account, DB1 leads, DB2 streets without a page; The One Thing as `rules.json`; small-sample floors) plus `.github/workflows/morning-report.yml` at 06:00 Toronto on a GitHub runner. Run once by hand and emailed (Resend `01a0c22b-4c51-72ef-b819-7f37b21acf84`), exit 0, no secret in any log, commit or the email. Waits on Core to merge `feat/audit` and add seven Actions secrets. Record in `scratchpad/reports/MA-008-morning-report.md`, state in `HANDOFF-audit.md`.*

*Out-of-queue work 2026-09-21, MA-007, why `/streets/[slug]` is 9.6% cached, AUDIT in the `D:miltonly-audit` worktree on `feat/audit`. Read-only against production at `d068f84`: the six candidates checked with file:line (no dynamic API in the tree, no header override, middleware falls through, 50 of 646 prerendered, repeated requests `HIT`), a sweep of all 646 street pages across a deploy (546 `MISS`), and the tooling share counted (about 68% of requests and 73% of renders are the battery and the nightly). Cause: deploy purges of the 596 non-prerendered streets, re-rendered by the battery; one-line fix for MC-035 at `src/lib/streetPrerender.ts:14`. Record in `scratchpad/reports/MA-007-street-cache-share.md`, state in `HANDOFF-audit.md`.*

*Out-of-queue work 2026-09-18, MA-006 addendum, the VOW-only fields AUDIT in the `D:miltonly-audit` worktree on `feat/audit`. Read-only against production at `e606d8b`: every per-listing VOW-only field rendered to an anonymous visitor (days on market and the list date it is derived from, prior price and price change, sold, expired or leased status on a still-visible listing, sold price), listed by surface with file:line: listing detail, `/listings` and its `status=` grids, `/rentals`, homepage, school and mosque pages, the three ad surfaces, the mega menu on every page, JSON-LD. Aggregates reported separately. Record in `scratchpad/reports/MA-006-vow-fields-addendum.md`, state in `HANDOFF-audit.md`.*

*Out-of-queue work 2026-09-16, MA-005, the neighbourhood hub page AUDIT in the `D:miltonly-audit` worktree on `feat/audit` (`origin/main` merged in first). Read-only against production at `1b2d7d8`: five hubs spanning the shapes (Timberlea, Bronte Meadows, Nassagaweya, Moffat, Ford) at 1440 and 390 with Puppeteer and Lighthouse, a cache sweep of all 22 hubs, every intent square and CTA card followed to its destination at 390, body links in from 1,198 sitemap URLs, and a benchmark against Zolo, HouseSigma, Realtor.ca and Rightmove. Twenty-five defects ranked (four S1: the June prose and FAQ figures contradicting the live tiles on all 22 hubs and shipping as `FAQPage` schema; the hub typical as `AVG` against the sold page median, with `/sold?nbhd=` losing the hub; no lead capture on the hub body; `bronte-meadows` registered `rural_hub`, which sets its SERP title and the rural sibling lists) and ten changes for core; nothing built. Not a queue item, prompted directly. Record in `scratchpad/reports/MA-005-hub-page-audit.md`, state in `HANDOFF-audit.md`.*

*Out-of-queue work 2026-09-13, MA-004, the header, mega menu and footer AUDIT in the `D:\miltonly-audit` worktree on `feat/audit`. Read-only against production: five page types (home, street, hub, listing, guide) at 1440, 1024 and 390 with Puppeteer and Lighthouse, every chrome href checked, footer coverage against the sitemap, the brief form submitted with its POST intercepted, and a benchmark against Homesly, Rightmove, Zoopla and Airbnb. Twenty-two defects ranked (four S1: the scrolled homepage bar at 390 loses the CTA and burger, no footer on 509 street pages, the panel CTA white on green on street pages, the navy chrome on 461 listing pages) and ten changes for core; nothing built. Not a queue item, prompted directly. Record in `scratchpad/reports/MA-004-nav-footer-audit.md`, state in `HANDOFF-audit.md`.*
...
*Out-of-queue work 2026-09-17: MP-001, the subscriber portal's Gate A recon, in the `D:\miltonly-portal` worktree on `feat/portal` (prefix `MP-`). Recon only, no code. What exists: a six-digit-code sign-in that ignores its `redirect` param and always lands on `/saved`, a 30-day JWT session, the VOW acknowledgement reachable on `/sold` only, `SavedSearch` written by Leads by email and by `/saved` by userId. `User` is 162 bot rows, 0 verified. Compliance table for TRREB VOW (the policy's "username and a password" clause against the no-password brief is the one ruling slice 1 waits on), PIPEDA and CASL (deal alerts carry no unsubscribe and no email carries a mailing address). Three slices proposed: MP-002 the door, MP-003 the account, MP-004 the loop back. State in `HANDOFF-portal.md`. Record in `scratchpad/reports/MP-001-portal-recon.md`.*

*Out-of-queue work 2026-09-18: MC-029, the TRREB VOW compliance hotfix, on `fix/vow-compliance @ de199c2` (code head `6c23bdc`), merged on local `main` as `5994cc7` on 2026-09-19 under the Vercel pause and gated locally; **pushed and on production 2026-09-20 as MC-030: production serves `c83fffb`, full battery `FAIL · 21 checks · 644 pages` on one creation-cron street only, purged and rerun `PASS · 4 checks · 645 pages`, `vow-fields` 15 of 15; record `scratchpad/reports/MC-030-vow-to-production.md`.** The seven VOW-only per-listing columns (`daysOnMarket`, `listedAt`, `priorPrice`, `priceChangedAt`, `lastPriceChangeAt`, `soldPrice`, `soldDate`) leave every anonymous surface through `src/lib/listings/vow.ts` (the public predicate and `stripVowFields`); an acknowledged session reads them from `/api/listings/[mls]/vow` on the listing page and on the grid's cards; sold, expired and leased listing pages answer the not-available shell, noindex; `/listings?status=sold` redirects to `/sold`; the brokerage renders inside the price on every card, the detail page and the alert email through one component. Battery `vow-fields` (red on main, 8 of 15; green on the branch), prebuild `test-vow-fields.ts` (67). Preview `czlzy3wc7` full battery `PASS · 21 checks · 609 pages · 769s`. Record in `scratchpad/reports/MC-029-vow-compliance.md`.*

*Out-of-queue work 2026-09-20: MC-031, MP-002b the password, `feat/portal @ 98d2cd7` merged by SHA as `c1caac8` (then `1a5a24d`, the battery's password sign-in). R-805(c): the email is the username, a 12+ character password with the acknowledgement, `POST /api/auth/login` the returning sign-in, `canSeeVowRecords` the one rule on ten files (MC-029's three brought under it in the merge). Full gate exit 0, migration 31 applied, production battery `PASS · 21 checks · 646 pages · 878s`; proven on production: the password-only card for an acknowledged row, a returning sign-in landing on the street's sold records. The desk passphrase lives in `.env.local` `VERIFY_PORTAL_PASSWORD`. Record `scratchpad/reports/MC-031-portal-password.md`.*

*Out-of-queue work 2026-09-20: MC-032, ML-005 the lead bot gate, `feat/leads @ 064c5b6` merged by SHA as `2510466` (three resolutions in the merge commit). Gate exit 0, prebuild `lead-bot-gate` 21; production battery `PASS · 21 checks · 646 pages · 871s`; proven on production: a quoted User-Agent writes no row and sends no email, a normal submission writes the row and both Resend sends deliver, the row deleted. Record `scratchpad/reports/MC-032-lead-bot-gate.md`.*

*Out-of-queue work 2026-09-20: MC-033, the cost check, no code. Vercel on-demand $156.76 at day 17 against $41.37 last cycle (build minutes, 71% down since the branch rule, projected about $210); Neon DB1 egress 71.6 GB month-to-date against the 16 GB ceiling, free on the plan, half of it the full-row `Listing` pull on street renders; Neon compute about $53 a month with a third on the integration project nothing reads. Record `scratchpad/reports/MC-033-cost-check.md`.*

*Out-of-queue work 2026-09-20: MC-034, the cost fixes, on `fix/cost-2 @ 6ba561e`, previewed (`miltonly-jm3fj4ug7`, battery `PASS · 21 checks · 646 pages · 802s`), **merged as `d068f84` on 2026-09-21 (target `edae874` by SHA), production battery `PASS · 21 checks · 663 pages · 770s`, the after-window 0 full-row pulls.** The street render's Listing rows: a fifteen-column select, React.cache per request, the Data Cache under the `listings` tag with a write stamp in the key (4 full-row pulls a render to 2 narrow ones cold and none warm, about 45 GB a month off DB1, $0). The ignore rule skips docs-only pushes on `main` from the last successful deployment (about $9 to $14 a cycle). The integration Neon project is inspectionly.ca's database and stays; suspend timeouts are already 300 s. One preview per task proposed (about $11.50 a cycle). Record `scratchpad/reports/MC-034-cost-fixes.md`.*

*Out-of-queue work 2026-09-21: MC-035, fewer deploys, merged as `7064c1a` (`fix/deploys @ 0b331ae`; the first build `722ad58` failed my own prebuild assertion and was fixed). DEC-BATCH-MERGE in CLAUDE.md; the battery's `--streets=sample` (default full until the nightly runs the full battery) and `prerender-coverage` check; every published street prerendered on production (796 static pages, build 145 s, first request 0.25 s PRERENDER); PetalBot, SemrushBot, Amazonbot and AhrefsBot refused in robots and by the Vercel Firewall. Battery `PASS · 22 checks · 677 pages · 746s`. No warm-streets job: the tag drops are the next lever. Record `scratchpad/reports/MC-035-fewer-deploys.md`.*

*Out-of-queue work 2026-09-21: MC-036, the PropTx VOW display rules, merged as `b814d99` (`fix/vow-display`; the first merge `1b18563` failed Vercel's prebuild on an uncommitted prompt doc). The address flag on every surface, the 100 cap, the bona fide notice on every page, the brokerage in the listing's typeface in every view, our contact separated, exclusives labelled, full-access claims reworded, `/privacy/request`, per-record lease comps out of the prompts, ShowingRequirements out of the sold sync (purge 006 prepared). Two battery checks, `vow-display` and `agent-only`. Open: Archive Data, PropTx's copyright text, the reliability sentence, two ToU clauses. Record `scratchpad/reports/MC-036-vow-display-rules.md`; sources in `docs/compliance/`.*

*Out-of-queue work 2026-09-21: MC-037, the VOW follow-ups, merged as `2411e8e` (`fix/vow-window @ 68f6662`), then `ecc4524` above it. The 24-month display window in one constant (`src/lib/vowWindow.ts`; DB2 keeps every row; the blast radius measured first: no street vanishes, 84 change tier, the existence gates stay whole), purge 006 executed (8,650 rows, 0 remain), the 8.25 reliability sentence beside the bona fide sentence on every page type including the three ads pages, the homepage lede, the condo generator's per-record rents out, the 19 legacy FAQs scrubbed, 22 hubs and 57 condos regenerated ($0.24). Battery `PASS · 24 checks · 690 pages · 591s`. The hub tier refuses "median" now. Open: Archive Data, "average" in hub and condo prose. Record `scratchpad/reports/MC-037-vow-window.md`.*

*Out-of-queue work 2026-09-22: MC-038, the batch under DEC-BATCH-MERGE: `feat/audit @ 6e99a86` (MA-008, the morning report) as `6d19346`, `feat/web-analytics @ 7cfa4a2` (MH-009) as `f1d4080`, then `1fee7f7`: the hub tier refuses "average" as it refuses "median" (15 hubs and 47 condos regenerated, $0.156; 158 Mill Street hand-scrubbed), millside-drive-milton hand-scrubbed. The morning report ran once by hand from `main` (run `35681404348`, email `01a0c70d…`) and committed `31da4b9`. Web Analytics proven on production (tokens redacted). Battery `PASS · 24 checks · 708 pages · 525s`. Record `scratchpad/reports/MC-038-batch-audit-analytics.md`.*

*Out-of-queue work 2026-09-22: MC-039, the MA-008 addendum, `feat/audit @ 9603468` merged by SHA as `c1565b7`: the morning report reads the $300 cap, projects spend plus the last seven full days' rate, and prints the whole street backlog. Nothing under `src/` or `prisma/`, so no local build and no battery. **The ignore rule does not skip an audit-only push** (it excludes only `scratchpad/`, `docs/` and `*.md`), so production rebuilt on identical app code; the fix is the rule as a repo script. Record `scratchpad/reports/MC-039-audit-addendum-merge.md`.*

*Out-of-queue work 2026-09-22: MC-040, the ignore rule out of `vercel.json` and into `scripts/vercel-ignore.sh` (29 characters in the config, from 250 against a 256 limit), with `scripts/audit/` added to the excluded paths so an audit-only push stops building production. Behaviour otherwise unchanged: `docs/phase-4.1/` always builds, the range is `VERCEL_GIT_PREVIOUS_SHA` and never the tip commit alone, everything uncertain builds. Runnable by hand; `scripts/test-vercel-ignore.ts` asserts 12 cases on real SHAs, including both halves of the MC-034 regression, and is last in the prebuild chain. Proven on Vercel: `886249e` (audit-only) Canceled, `ad8ae77` and `bfc389b` Ready. Record `scratchpad/reports/MC-040-ignore-rule-in-a-file.md`.*

*Out-of-queue work 2026-09-23: MC-042, three street pages from the backlog (gowland-crescent, ennisclare-drive, jempson-path; registry and Town geometry real for all three). Generated through the standing runners, DeepSeek only, $0.0310, and published. While live they passed 73 of 73 production assertions (both VOW sentences, the 24-month window, withheld addresses, the 100 cap) and the battery (`PASS · 24 checks · 722 pages · 482s`), and the audit's own runner no longer found them. **Returned to draft** after a fabrication review found 32 served claims with no ground in the generator input (compass position, land use, housing mix, build era); production is as before, and the decision to republish or fix the grounding first is open. Record `scratchpad/mc042/MC-042-three-street-pages.md`.*

*Out-of-queue work 2026-09-24: MC-043, MA-010's compliance fixes, with MA-010 merged by SHA (`794ef5a` as `60295f2`) in the same build. /about's award block is two cards, "RE/MAX Hall of Fame" and "Serving Milton Since 2011"; the "only" claim leaves config.ts, layout.tsx and schema.ts for "Built exclusively for Milton, Ontario"; "RE/MAX Realty Specialists Inc., Brokerage" renders in full wherever the brokerage names itself, our own listings' "Listed by" line included; the homepage's schema-only FAQPage is removed. Local gate exit 0, local, preview and production batteries `PASS · 24 checks · 719 pages`; production serves `5f8cdb0`. Open: the award list outside /about. Record `scratchpad/mc043/MC-043-ma010-compliance-fixes.md`.*

*Out-of-queue work 2026-09-24: MC-045, the fabrication measured on the served HTML of all 719 street pages, measure only. Build-era claims on 521 of 707 pages with generated text (1,173 sentences, precision 89.9%, recall 84.1%); compass claims WRONG against Town geometry on 56 pages; self-contradictions on 38 (a floor); 543 affected pages hold 63% of street clicks; hubs and condos share it. Nothing on production changed. The CRLF env-loader fix and the ignore test's branch leak are on `fix/env-loader-crlf @ 4cf16e4`, held for the next batch. Records `scratchpad/mc045/MC-045-fabrication-measure.md` and `scratchpad/mc045/MC-045-env-loader-fix.md`.*

*Out-of-queue work 2026-09-25: MC-044, the batch under DEC-BATCH-MERGE, one production build. `feat/portal @ fd65058` (MP-006) as `601c64a`, `fix/env-loader-crlf @ 4cf16e4` (MC-045) as `6b2cfe7`, `feat/leads @ a001c43` (ML-012) as `6e14465`; MH-009 `7cfa4a2` already on `main` since MC-038. Core fixes `be8e127` (one deletion channel on /privacy; `vow-fields` carries the session /me re-issues) and `b60aa11` (`*.sh` LF, without which no worktree preview passes prebuild). Gate exit 0 at each SHA; local battery `PASS · 24 checks · 719 pages · 429s`; preview `miltonly-knzmybnxv` from a worktree branch Ready; production battery `PASS · 24 checks · 719 pages · 559s`. Proven on production: the v4 card's ten clauses with ix bold, a `VowAccessLog` row from a real signed-in read, the export's 401 and CSV, the 60-minute timeout in real time; a headed view recorded by Web Analytics in 37 s; the rental fields at 4.89:1 and 0 links 404. Record `scratchpad/mc044/MC-044-batch-merge.md`.*

*Out-of-queue work 2026-09-18: MC-028, four merges by SHA in order, each behind the full local gate (exit 0 on Node 22): `fix/favicon @ 9144b35` as `ea81883`; **Street Page v3 (MH-005) DONE**, `feat/street-v3 @ 2553f2e` merged as `e078e91`; **Portal slice 1 (MP-002, the door) DONE**, `feat/portal @ 86d5f9f` merged as `706ce07`, `prisma migrate status` clean at 30 (the `portal_door` migration was already applied); **ML-004 DONE**, `feat/leads @ 7eeeb79` merged as `7196623`. Production serves `7196623`, battery `PASS · 20 checks · 609 pages · 713s`. Confirmed there: the Miltonly favicon, the hero capture field and the ladder number box on main-street, sign-in returning to the street, one brief with the CASL footer. Lighthouse mobile on main-street perf 90, SEO 100, a11y 100. Record in `scratchpad/reports/MC-028-four-merges.md`.*

*Out-of-queue work 2026-09-17: MP-002, the portal's slice 1, the door, in `D:\miltonly-portal` on `feat/portal`, app code `86d5f9f`. The 162 bot `User` rows purged (dry run, then apply, 0 kept). `/api/auth/signup` behind the lead layer's honeypot, origin check and Upstash rate limit, in that order; the email carries a magic link and the same 6-digit code, 15 minutes, five attempts, constant-time compare; `redirect` honoured as a same-origin path; the VOW card inline on the street page for a signed-in, unacknowledged person, asking name and home street (registry autocomplete), storing the consent pair on `User`; session fixed at 90 days; passwordless noted in the VOW text (version 2) and `/terms` pending the broker of record. Migration `20260917120000_portal_door` applied through the ledger. Prebuild `scripts/test-portal-door.ts`: 100 bot signups send 0 emails. Proven on preview `miltonly-438g01i85` at 390 with an iPhone UA: gate → email in 13 s → link → that street's sold records, acknowledged, 12 rows, 4.0 s. NOT merged; Core merges by SHA. Record in `scratchpad/reports/MP-002-the-door.md`.*

*Out-of-queue work 2026-09-18: MP-002b, the password, in `D:\miltonly-portal` on `feat/portal`, app code `98d2cd7`. The broker of record ruled under TRREB R-805(c): a username and a password per consumer. The email is the username; the card asks for a password (12+ characters, not the email, bcrypt cost 12) after the link or code verifies the email; returning sign-in is email + password (`/api/auth/login`, its own rate limit, one message for wrong, unknown and unset) with "Email me a link instead" as the fallback; no VOW record is served until the password exists (`src/lib/vow-access.ts`, called by all seven VOW surfaces); the session stays a fixed 90 days; VOW text version 3 names username and password. Migration `20260918120000_portal_password` applied through the ledger. Prebuild 120 assertions. Proven on preview `miltonly-g8swlwol7` at 390 with an iPhone UA: link then the full card (12 rows, 4.7 s), password sign-in with no card (12 rows, 3.0 s), a wrong password refused, the password-only card for an acknowledged row. NOT merged; Core merges by SHA. Record in `scratchpad/reports/MP-002b-the-password.md`.*

*2026-09-19, MP-002b under the Vercel pause: every deployment answers 402, so the branch head `f7ddf4b` was gated locally (build exit 0 on Node 22; `next start -p 3111` serving the local SHA; battery `PASS · 20 checks · 626 pages` after dropping the stale on-disk Data Cache, which had made the first run report yesterday's figures on two hubs; the four phone flows repeated on localhost). Gated locally, preview pending. Still NOT merged; Core merges when production answers 200.*

*Out-of-queue work 2026-09-20: MP-002c, the User-Agent guard on the door, in `D:\miltonly-portal` on `feat/portal`, app code `f7813c1`. `requestSignIn()` runs ML-005's `checkUserAgent` after the honeypot and before the origin check: a missing or quoted User-Agent answers 200 with the success words, writes nothing, sends nothing, spends no limiter token, and the refused value is logged. `src/lib/lead/guards.ts` on the branch is `origin/feat/leads @ 064c5b6` verbatim so the file merges clean in either order. Prebuild 155 assertions. Gated locally (build exit 0, battery `PASS · 21 checks · 646 pages` on localhost, the guard probed with curl on the running server); a 61-agent adversarial review confirmed 19 findings, six fixed in the slice, the first open one being that `/api/auth/verify` has no guard at all (pre-existing, proposed MP-002d). No preview, by the task's word. NOT merged; Core merges by SHA. Record in `scratchpad/reports/MP-002c-user-agent-guard.md`.*
