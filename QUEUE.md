...
*Out-of-queue work 2026-09-21: MP-006, the VOW credential and access rules from the PropTx Best Practices, in `D:\miltonly-portal` on `feat/portal`, app code `fd65058`. Read against MP-002 and MP-002b first: the 90-day limit was on the session, not the password; no inactivity timeout; the 180-day record rule was prose; two of the nine Terms of Use clauses were present; no audit trail; no registrant exclusion or tracking. Built: the 90-day password with renewal (R-8.06), the 60-minute inactivity timeout on the one session cookie (R-8.13), the 180-day credential-record rule, the terms at version 4 with the nine clauses and re-consent with a VowConsent history (Appendix B(c)), the VowAccessLog trail on eight surfaces with an admin export and a shell export (VOW Policy 19), the registrant question and review-only suspicious-activity tracking (R-8.09(b)), the PropTx sentence in bold on /privacy. The bcrypt hash stays; the "current password" wording is a question for PropTx. A 204-agent review confirmed 64 findings, 30 fixed. Prebuild 202 + 161 assertions. Gated: build exit 0, local battery `PASS · 22 checks · 690 pages`, one preview `miltonly-rl47fc9cp` proven at 390 (the card, the records, the export, the touch, the re-consent, the renewal). Every existing session signs out at deploy and every row owes re-consent; the desk's row is current; its password expires 2026-12-19 and the battery cannot renew it. NOT merged; Core merges by SHA. Record in `scratchpad/reports/MP-006-vow-best-practices.md`.*
