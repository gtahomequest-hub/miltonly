...
- `vercel.json` `ignoreCommand` lets a Git-triggered build run only on `main`, and on `main` only when the commit touches something outside `scratchpad/`, `docs/` and `*.md` (MC-034); `docs/phase-4.1/` is the exception and always builds, because the generators read those prompt files at runtime. A push to any other branch is cancelled; a branch gets its preview from `npx vercel` in its worktree (CLI deploys do not run the command). A docs-only `main` commit (HANDOFF, QUEUE, a report, the nightly audit) does not build; anything under `src/`, `scripts/`, `prisma/`, `public/` or a config file does. Never put app input under `scratchpad/` or `docs/` outside `phase-4.1/`.
